        ...
        if (weapon) {
            ...
            if ((std::find(std::begin(pistols), std::end(pistols),
                ...
                semiAuto = true;
                             ...
                             weapon->itemIndex() & 0xFFF) != std::end(heavyWeapons)) &&
            ...
            // Enumerate over players and get angle to the closest player hitbox (that is
            // enabled) to crosshair
            for (int i = 1; i < Interfaces::globals->maxClients; i++) {
                Player* p = (Player*)Interfaces::entityList->GetClientEntity(i);
                if (p && p != Globals::localPlayer) {
                    if (p->health() > 0 && !p->dormant() && p->isEnemy() && p->visible() && !p->hasImmunity()) {
                                ...
                                for (int i = 0; i < 5; i++) {  // swap this for for loop of
                                    ...
                                    if (hitboxes & 1 << i) {
                                        ...
                                        const float curDistance = std::clamp(
                                             (targetBonePos - localPlayerEyePos).Length(),
                                             minDistance, FLT_MAX);
                                        if (distanceFOV)
